...
import { filesCdnBase, getPublishedFiles, committedFileCount } from '@/lib/published-files';
...
export async function GET() {
  ...
  //    every file link simply stays on Canvas. What matters is distinguishing
  //    "reading the current map from the host" from "falling back to whatever
  //    was last committed", because both serve pages and only one is fresh.
  ...
  if (!cdn) {
    ...
    const { count, source } = await getPublishedFiles();
    if (source === 'host') {
      checks.push({
        name: 'course-files',
        ok: true,
        detail: `${count} file(s) published via ${cdn}`,
      });
    } else {
      checks.push({
        name: 'course-files',
        ok: false,
        detail: `${cdn} is unreachable; using the committed map ` +
                `(${committedFileCount()} file(s)). Newer files fall back to Canvas.`,
      });
      status = worst(status, 'warning');
    }
  }
