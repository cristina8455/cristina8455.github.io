...
import { publishedFileUrl } from '@/lib/published-files';
 ...
 * pointing at Canvas.
 *
 *   1. sanitize  — drop <link>/<style>/<script>. Canvas embeds the account's
 *                  DesignPlus stylesheet in page bodies, and injected that way
 *                  it styles the whole document, not the content area.
 ...
 * which step 4 then handles. The page renders correctly with none of them.
 */
export function prepareCanvasHtml(html: string, ctx?: LinkRewriteContext): string {
  let out = sanitizeCanvasHtml(html);
  if (ctx) out = rewriteCanvasLinks(out, ctx);
  out = rewriteCanvasFileLinks(out, publishedFileUrl);
